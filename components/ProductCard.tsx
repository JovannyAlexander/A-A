...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const stockClass =
    product.stock > 10 ? 'text-green-600 bg-green-100' : product.stock > 5 ? 'text-yellow-600 bg-yellow-100' : 'text-red-600 bg-red-100'
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all min-w-0 w-full">
      ...
      <div className="p-4 sm:p-5 min-w-0">
        ...
        <button
          ...
          className={`w-full py-2.5 sm:py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all ${
            product.stock === 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600 text-white shadow-lg'
          }`}
        >
